...
void MainWindow::enableButtonsBasedOnSequence() {
    ...
    bool showPageButtons = hasSeq && 
        (currentSeq->IsFinite() ? currentSeq->GetLength() > DISPLAY_SIZE : true);
    ...
    if (showPageButtons) {
        ...
        if (currentSeq->IsFinite()) {
            size_t maxPage = (currentSeq->GetLength() + DISPLAY_SIZE - 1) / DISPLAY_SIZE;
            ui->pushButton_right->setEnabled(currentOffset < maxPage - 1);
        }
    }
}
...
void MainWindow::on_pushButton_5_clicked() {
    ...
    if (selected == actNewFinite) {
        ...
        if (infChoice == actArithmetic) {
            ...
            if (!ok) return;
            auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(
                Function<double(size_t)>([first, step](size_t i) { return first + step * i; })
            ));
            setCurrentSequence(newSeq, true);
            ...
            auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(
                Function<double(size_t)>([rng = LCG(static_cast<unsigned>(std::time(nullptr)))] (size_t) mutable -> double {
            ...
            if (!ok) return;
            auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(
                Function<double(size_t)>([value](size_t) { return value; })
            ));
            setCurrentSequence(newSeq, true);
            ...
            auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(
                Function<double(size_t)>([n = 1ULL, fact = 1.0] (size_t) mutable -> double {
                    fact *= n;
                    ++n;
                    return fact;
                })
            ...
            auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(
                Function<double(size_t)>([a = 0.0, b = 1.0] (size_t) mutable -> double {
                    double cur = a;
                    double nxt = a + b;
                    a = b;
                    b = nxt;
...
void MainWindow::on_pushButton_7_clicked() {
    ...
    if (!currentSeq->IsFinite()) {
        auto oldSeq = currentSeq;
        auto newFunc = [oldSeq, mapper](size_t i) -> double {
            return mapper(oldSeq->Get(i));
        };
        auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(Function<double(size_t)>(newFunc)));
...
void MainWindow::on_pushButton_left_clicked() {
    qDebug() << "*** left clicked, currentOffset =" << currentOffset;
    if (!currentSeq) return;
    if (currentOffset > 0) {
        --currentOffset;
        updateTable();
...
void MainWindow::on_pushButton_right_clicked() {
    qDebug() << "*** right clicked, currentOffset =" << currentOffset;
    if (!currentSeq) return;
    if (currentSeq->IsFinite()) {
        size_t maxPage = (currentSeq->GetLength() + DISPLAY_SIZE - 1) / DISPLAY_SIZE;
        if (currentOffset + 1 < maxPage) {
            ++currentOffset;
            updateTable();
            logMessage(QString("Диапазон: %1 – %2")
                           .arg(currentOffset * DISPLAY_SIZE)
                           .arg(std::min((currentOffset + 1) * DISPLAY_SIZE - 1, currentSeq->GetLength() - 1)));
            ui->pushButton_left->setEnabled(currentOffset > 0);
            ui->pushButton_right->setEnabled(currentOffset < maxPage - 1);
        }
    } else {
...
void MainWindow::on_tableWidget_cellChanged(int row, int column) {
    qDebug() << "*** cellChanged row=" << row << "col=" << column << " currentOffset=" << currentOffset;
    if (!currentSeq || !currentSeq->IsFinite()) {
    ...
    size_t globalIndex = currentOffset * DISPLAY_SIZE + static_cast<size_t>(row);
    ...
    qDebug() << "  savedOffset =" << savedOffset;
    ...
    auto newSeq = SharedPtr<LazySequence<double>>(new LazySequence<double>(values.begin(), values.size()));
    setCurrentSequence(newSeq, false, true);

    if (savedOffset * DISPLAY_SIZE < newSeq->GetLength()) {
        currentOffset = savedOffset;
    } else {
        currentOffset = 0;
    }
    qDebug() << "  after restore, currentOffset =" << currentOffset;
...
void MainWindow::updateInfoPanel() {
    ...
    if (currentSeq->IsFinite()) {
        ...
        info += "Текущий диапазон: " + QString::number(currentOffset * DISPLAY_SIZE) + " – " +
                QString::number(std::min((currentOffset + 1) * DISPLAY_SIZE - 1, currentSeq->GetLength() - 1));
    } else {
...
void MainWindow::updateTable() {
    qDebug() << "*** updateTable: currentOffset =" << currentOffset;
    
    int savedScrollPos = 0;
    if (ui->tableWidget->verticalScrollBar()) {
        savedScrollPos = ui->tableWidget->verticalScrollBar()->value();
        qDebug() << "    savedScrollPos (before clear) =" << savedScrollPos;
    }
    ...
    size_t rows = 0;
    if (currentSeq->IsFinite()) {
        size_t len = currentSeq->GetLength();
        if (currentOffset * DISPLAY_SIZE >= len) {
            currentOffset = 0;
            qDebug() << "    -> resetting offset to 0 because start >= length";
        }
        start = currentOffset * DISPLAY_SIZE;
        rows = std::min(DISPLAY_SIZE, len - start);
    } else {
        start = currentOffset * DISPLAY_SIZE;
        rows = DISPLAY_SIZE;
    }
    ...
    if (rows > 0) {
        int targetScroll = savedScrollPos;
        QTimer::singleShot(0, [this, targetScroll]() {
            if (ui->tableWidget->verticalScrollBar()) {
                int maxScroll = ui->tableWidget->verticalScrollBar()->maximum();
                if (targetScroll <= maxScroll) {
                    ui->tableWidget->verticalScrollBar()->setValue(targetScroll);
                    qDebug() << "    restored scroll to" << targetScroll;
                } else {
                    ui->tableWidget->verticalScrollBar()->setValue(maxScroll);
                    qDebug() << "    restored scroll to max" << maxScroll;
                }
            }
        });
    }
...
void MainWindow::setCurrentSequence(SharedPtr<LazySequence<double>> newSeq, bool addToHistory, bool preserveOffset) {
    qDebug() << "*** setCurrentSequence: preserveOffset =" << preserveOffset << " old offset =" << currentOffset;
    if (!newSeq) {
    ...
    if (!preserveOffset) {
        currentOffset = 0;
        qDebug() << "    -> reset offset to 0 (preserveOffset false)";
    } else {
        if (newSeq->IsFinite()) {
            size_t maxOffset = (newSeq->GetLength() + DISPLAY_SIZE - 1) / DISPLAY_SIZE;
            if (currentOffset >= maxOffset) {
                currentOffset = 0;
                qDebug() << "    -> reset offset because out of range";
            } else {
                qDebug() << "    -> kept offset = " << currentOffset;
            }
        } else {
            qDebug() << "    -> kept offset = " << currentOffset;
        }
...
void MainWindow::applyOperation(const QString& opName, SharedPtr<LazySequence<double>> newSeq, bool addToHistory) {
    ...
    setCurrentSequence(newSeq, addToHistory, true);
...
void MainWindow::replaceCurrentSequence(SharedPtr<LazySequence<double>> newSeq, const QString& opName, bool preserveOffset) {
    qDebug() << "*** replaceCurrentSequence: preserveOffset =" << preserveOffset << " old offset =" << currentOffset;
    if (!newSeq) {
        logMessage("Ошибка: " + opName + " не выполнена");
        return;
    }
    ...
    if (preserveOffset) {
        if (!newSeq->IsFinite()) {
            currentOffset = savedOffset;
            qDebug() << "    -> kept offset (infinite) = " << currentOffset;
        } else {
            size_t maxOffset = (newSeq->GetLength() + DISPLAY_SIZE - 1) / DISPLAY_SIZE;
            if (savedOffset >= maxOffset) {
                ...
                currentOffset = savedOffset;
                qDebug() << "    -> kept offset (finite) = " << currentOffset;
            }
        }
    } else {
        currentOffset = 0;
        qDebug() << "    -> reset offset to 0";
    }
    currentSeq = newSeq;
